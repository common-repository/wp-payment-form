...
/***/ "./resources/public/moneris_handler.js":
/*!*********************************************!*\
  !*** ./resources/public/moneris_handler.js ***!
  \*********************************************/
/*! no exports provided */
...
var MonerisHandler = /*#__PURE__*/function () {
  function MonerisHandler($that, config) {
    _classCallCheck(this, MonerisHandler);
    this.$form = $that.form;
    this.payformHandler = $that;
    this.config = config;
    this.formId = config === null || config === void 0 ? void 0 : config.form_id;
    this.generalConfig = window.wp_payform_general;
  }
  _createClass(MonerisHandler, [{
    key: "init",
    value: function init() {
      var _this = this;
      this.$form.on('wppayform_next_action_moneris', function (event, data) {
        var response = data.response;
        console.log('moneris response', response);
        jQuery('<div/>', {
          'id': 'form_success',
          'class': 'wpf_form_notice_success wpf_moneris_text'
        }).html(response.data.message).insertAfter(_this.$form);
        if (response.data.actionName === 'initMonerisModal') {
          _this.initMonerisCheckout(response.data);
        } else {
    ...
    key: "initMonerisCheckout",
    value: function initMonerisCheckout(res) {
      var that = this;
      ...
      // // omneris div
      // var monerisCheckoutDiv = jQuery('<div/>', {
      //     id: 'monerisCheckout'
      // });

      var outerDiv = jQuery.parseHTML("\n            <div id=\"outerDiv\" style=\"width:400px\"; height\"300px\">\n                <div id=\"monerisCheckout\"></div>\n            </div>\n        ");
      outerDiv = jQuery(outerDiv);

      // Append the inner div to the outer div
      //outerDiv.append(monerisCheckoutDiv);
      // Insert the outer div after the form
      console.log('outerDiv', outerDiv);
      outerDiv.insertAfter(this.$form);
      var checkoutData = res.checkout_data;
      var metData = checkoutData['metadata'];
      var ticket = checkoutData['ticket'];
      var myCheckout = new monerisCheckout();
      myCheckout.setMode(checkoutData['environment']);
      myCheckout.setCheckoutDiv("monerisCheckout");
      myCheckout.startCheckout(ticket);

      // on moneris page loaded
      myCheckout.setCallback("page_loaded", function (res) {
        console.log("Moneris page loaded");
      });
      // on moneris page cancelled/returned
      myCheckout.setCallback("cancel_transaction", function (res) {
        console.log("Moneris transaction cancelled");
        outerDiv.remove();
      });
      myCheckout.setCallback("error_event", function (res) {
        console.log("Moneris error", res);
      });
      myCheckout.setCallback("payment_complete", function (res) {
        console.log('payment complete', res);
        outerDiv.remove();
      });
      myCheckout.setCallback("payment_receipt", function (res) {
        var jsonResponse = JSON.parse(res);
        console.log('payment receipt', jsonResponse, jsonResponse.ticket, _typeof(jsonResponse));
        var data = _objectSpread({
          action: 'wppayform_moneris_confirm_payment',
          form_id: that.formId,
          transaction_id: metData.transaction_id,
          submission_id: metData.submission_id
        }, jsonResponse);
        that.$form.parent().find('.wpf_moneris_text').remove();
        that.payformHandler.handleStripePaymentConfirm(data);
        outerDiv.remove();
      });
    ...
    key: "myPaymentComplete",
    value: function myPaymentComplete(response) {
      console.log('payment complete', response);
    }
  }]);
  return MonerisHandler;
}();
(function ($) {
  $.each($('form.wppayform_has_payment'), function () {
    var $form = $(this);
    $form.on('wppayform_init_single', function (event, $that, config) {
      new MonerisHandler($that, config).init();
    });
  });
})(jQuery);
...
/***/ 10:
/*!***************************************************!*\
  !*** multi ./resources/public/moneris_handler.js ***!
  \***************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Volumes/projects/wordpress/wp-content/plugins/wp-payment-form/resources/public/moneris_handler.js */"./resources/public/moneris_handler.js");
